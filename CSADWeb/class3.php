<?php
    ...
    if (isset($_POST['save'])) {
        ...
        $query = "SELECT class3 FROM users";
        $res = mysqli_query($conn, $query);
        while($row = mysqli_fetch_array($res)) {
            if ($row['class3'] == 0) {
                $conn->query("INSERT INTO stars_class3 (rateIndex) VALUES ('$ratedIndex')");
                $sql = $conn->query("SELECT id FROM stars_class3 ORDER BY id DESC LIMIT 1");
                $uData = $sql->fetch_assoc();
                $uID = $uData['id'];

                //var_dump($num);
                $class3 = "UPDATE users SET class3='1' WHERE email='$email'"; 
                mysqli_query($conn, $class3);
            } else {
                $conn->query("UPDATE stars_class3 SET rateIndex='$ratedIndex' WHERE id='$uID'");
            }
        }
    ...
    $result = $conn->query("SELECT count(*) as total from stars_class3");
    $idData = $result->fetch_assoc();
    $idTotal = $idData['total'];
    
    $sql = $conn->query("SELECT id FROM stars_class3");
    $numR = $sql->num_rows;

    $sql = $conn->query("SELECT SUM(rateIndex) AS total FROM stars_class3");
    $rData = $sql->fetch_array();
    $total = $rData['total'];

    $avg = $total / $numR;
    
    require 'files_class3.php';
    $file = new files;
    ...
    if(isset($_POST['submit'])) {
        ...
        if($file->insert()) {
            if(move_uploaded_file($_FILES['file']['tmp_name'], 'images_class3/' . $_FILES['file']['name'])) {
    ...
    <body>
        ...
        <main>
            <div class="class-upper-container">
                ...
                <div class="class-title">Internet Fundamentals</div>
            ...
            <div id="container">
                ...
                <div class="class-card">
                    <div class="slider img-gallery">
                        <?php
                            echo "<div class='image'><img src='images_class3/bottom-pic-3.jpg'></div>";
                            foreach($images as $image) {
                                echo "<div class='image'><img src='images_class3/" . $image["name"] . "'></div>";
                            }
                        ?>
                    </div>
